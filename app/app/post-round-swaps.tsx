...
export function PostRoundSwaps({ groupMemberId, roundId, roundName, players, confirmedSwaps, maxSwaps }: Props) {
  ...
  return (
    <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
      {/* Cabeçalho */}
      <div className="px-4 py-3 bg-gray-900/60 border-b border-gray-700 flex items-center justify-between">
        <div>
      ...
      {/* Aviso inicial */}
      {!alreadyConfirmed && !done && (
        <div className="px-4 py-2 bg-lime-500/10 border-b border-lime-500/20">
      ...
      {(alreadyConfirmed || done) && (
        <div className="px-4 py-3 border-b border-gray-700">
          <p className="text-xs text-gray-400 mb-2">Trocas confirmadas:</p>
        ...
        <div className="p-4 space-y-4 max-h-[500px] overflow-y-auto">
                      ...
                      <div key={p.player_id}>
                        ...
                        >
                          ...
                          <div className="flex flex-col items-end gap-0.5 shrink-0">
                            ...
                            {canSelect && !isPendingOut && (
                              <span className="text-[9px] text-gray-500">clique para trocar</span>
                            )}
                          </div>
                        </div>
                        ...
                        {isPendingOut && replacingPlayer && (
                          <div className="ml-6 mt-1 flex items-center gap-1.5 text-[10px] text-lime-400">
                            ...
                            <span className="font-medium">{replacingPlayer.name}</span>
                        ...
                        {isSelected && eligibleBench.length > 0 && (
                          <div className="mt-1.5 ml-2 space-y-1 border-l-2 border-lime-400/40 pl-3">
                            <p className="text-[10px] text-lime-400/70 mb-1">Escolha um reserva:</p>
                            {eligibleBench.map(b => (
                              <button
                                key={b.player_id}
                        ...
                        {isSelected && eligibleBench.length === 0 && (
                          <p className="mt-1 ml-2 text-[10px] text-gray-500 italic">
                            Nenhum reserva elegível (precisa ter nota maior na mesma posição)
                          </p>
                        )}
                      </div>
                    )
                        ...
                        <div
                          ...
                          className={`flex items-center gap-2.5 px-3 py-1.5 rounded-lg border ${
                            isPendingIn
                              ...
                              : 'bg-gray-700/20 border-gray-700/30 opacity-60'
                          }`}
                        >
                          <PlayerAvatar player={p} size="sm" />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1">
                              <span className="text-gray-400 text-[9px] font-bold bg-gray-600 px-1 rounded">BANCO</span>
          ...
          {pendingSwaps.length > 0 && (
            <button
              ...
              disabled={loading}
              className="w-full py-2.5 bg-lime-500 hover:bg-lime-400 text-gray-900 font-bold text-sm rounded-lg transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <span>Confirmando...</span>
              ) : (
                <span>✓ Confirmar {pendingSwaps.length} troca{pendingSwaps.length !== 1 ? 's' : ''} e atualizar score</span>
              )}
            </button>
          )}
        </div>

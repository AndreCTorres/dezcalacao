...
function fuzzyMatch(query, str) {
  ...
  // Last name match (útil para 'Vinicius Jr' vs 'Vinícius Júnior')
  const queryParts = q.split(/[\s\-]/).filter(p => p.length > 2)
  const strParts = s.split(/[\s\-]/).filter(p => p.length > 2)
  
  let matches = 0
  for (const qPart of queryParts) {
    if (strParts.some(sPart => sPart.includes(qPart) || qPart.includes(sPart))) {
      matches++
    }
  }
  
  if (matches > 0) {
    return 0.7 + (matches / queryParts.length) * 0.3
  }
...
async function run() {
  ...
  if (!groupId || !roundId) {
    console.error('❌ Uso: node scripts/seed-round1-ratings-fuzzy.mjs <groupId> <roundId>')
    process.exit(1)
  }

  console.log(`🚀 Populando Rodada 1 com fuzzy matching para grupo ${groupId}...`)

  try {
    // 1. Buscar todos os jogadores do banco em memoria (cache)
    console.log('\n📥 Carregando banco de jogadores em memória...')
    ...
    // Agrupar por team para buscas rápidas
    const playersByTeam = {}
    ...
    // 3. Inserir ratings com fuzzy matching
    console.log('\n🎯 Inserindo ratings (fuzzy matching)...')
    let totalRatings = 0
    let totalErrors = 0
    ...
    for (let i = 0; i < ROUND1_DATA.length; i++) {
      ...
      // Buscar IDs dos times para essa partida
      const homeTeamId = allPlayers.find(p => p.team_id && playersByTeam[p.team_id]?.some(t => t.name))?.team_id
      const awayTeamId = allPlayers.find(p => p.team_id && playersByTeam[p.team_id]?.some(t => t.name))?.team_id

      // Procurar por team_name
      const { data: homeTeam } = await supabase
        .from('teams')
        .select('id')
        .ilike('name', `%${match.home}%`)
        .limit(1)
        .single()
      ...
      const { data: awayTeam } = await supabase
        .from('teams')
        .select('id')
        .ilike('name', `%${match.away}%`)
        .limit(1)
        .single()

      if (!homeTeam || !awayTeam) {
        console.log(`    ⚠️  Times não encontrados para ${match.label}`)
        continue
      }
      ...
      // Buscar jogadores de ambos os times
      const { data: homeTeamPlayers } = await supabase
        .from('players')
        .select('id, name')
        .eq('team_id', homeTeamPlayersId)
        .eq('season', 'WC2026')

      const { data: awayTeamPlayers } = await supabase
        .from('players')
        .select('id, name')
        .eq('team_id', awayTeamPlayersId)
        .eq('season', 'WC2026')

      const allTeamPlayers = [...(homeTeamPlayers || []), ...(awayTeamPlayers || [])]

      // Para cada rating, fazer fuzzy matching
      for (const playerRating of match.ratings) {
        const bestMatch = findBestMatch(playerRating.name, allTeamPlayers)

        if (!bestMatch || bestMatch === null) {
          totalErrors++
          console.log(`    ⚠️  ${playerRating.name} não encontrado (fuzz)`)
          continue
        }

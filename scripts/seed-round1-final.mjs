...
// Simples função de fuzzy match com Levenshtein
...
function findBestMatch(name, candidates, minScore = 0.6) {
  let best = null
...
// Mapeamento de nomes de times (do script para o banco)
...
async function run() {
  ...
  if (!groupId || !roundId) {
    console.error('❌ Uso: node scripts/seed-round1-final.mjs <groupId> <roundId>')
    process.exit(1)
  }

  console.log(`🚀 Inserindo ratings finais com Levenshtein fuzzy match...\n`)

  try {
    // 1. Buscar TODOS os jogadores
    console.log('📥 Carregando todos os 1000+ jogadores...')
    const { data: allPlayers, error: playerError } = await supabase
      .from('players')
      .select('id, name, team_id')
      .eq('season', 'WC2026')

    if (playerError || !allPlayers) {
    ...
    // 2. Agrupar por team_id para buscas rápidas
    ...
    // 3. Pre-fetch de team IDs por nome
    console.log('📥 Pré-carregando IDs de times...')
    ...
    if (teamError) {
      console.error('❌ Erro ao buscar times:', teamError)
      process.exit(1)
    }

    const teamsByCountry = {}
    for (const team of teams) {
      teamsByCountry[team.country] = team.id
    }

    console.log(`  ✓ ${teams.length} times carregados`)

    // 4. Criar todos os fixtures
    console.log('\n📋 Criando fixtures...')
    const fixtures = []
    ...
    for (const match of ROUND1_DATA) {
      ...
      if (error) {
        console.log(`  ✗ Erro ao criar ${match.label}: ${error.message}`)
        continue
      }
    ...
    // 5. Inserir ratings com fuzzy matching global
    console.log('\n🎯 Inserindo ratings com Levenshtein distance...')
    let totalRatings = 0
    let totalErrors = 0
    ...
    for (let i = 0; i < ROUND1_DATA.length; i++) {
      ...
      if (!fixture) {
        console.log(`  ⚠️  Pulando ${match.label}`)
        continue
      }
      ...
      // Buscar times
      const homeTeamId = teamsByCountry[match.home]
      const awayTeamId = teamsByCountry[match.away]

      if (!homeTeamId || !awayTeamId) {
        console.log(`    ❌ Times não encontrados (${match.home} ou ${match.away})`)
        continue
      }

      // Buscar jogadores dos dois times
      const homeTeamPlayers = playersByTeamId[homeTeamId] || []
      const awayTeamPlayers = playersByTeamId[awayTeamId] || []
      const allTeamPlayers = [...homeTeamPlayers, ...awayTeamPlayers]

      console.log(`    ${homeTeamPlayers.length + awayTeamPlayers.length} jogadores disponíveis`)

      for (const playerRating of match.ratings) {
        const bestMatch = findBestMatch(playerRating.name, allTeamPlayers, 0.5)
    ...
    if (totalRatings + totalErrors > 0) {
      const taxa = (totalRatings / (totalRatings + totalErrors) * 100).toFixed(1)

/**
 * Script para popular Rodada 1 com fixtures e ratings
 * Uso: npx ts-node scripts/seed-round1-ratings.ts <groupId> <roundId>
 * 
 * Cria os 7 fixtures da Rodada 1 da Copa 2026 e insere todas as notas dos jogadores
 */
...
async function run() {
  ...
  if (!groupId || !roundId) {
    console.error('❌ Uso: npx ts-node scripts/seed-round1-ratings.ts <groupId> <roundId>')
    process.exit(1)
  }
  ...
  try {
    ...
    for (let i = 0; i < ROUND1_DATA.length; i++) {
      ...
      for (const playerRating of allPlayers) {
        // Normalize nome para busca fuzzy
        const searchName = playerRating.name.toLowerCase().trim()

        // Query: buscar jogador por nome (como Supabase faz, full text search não funciona bem aqui)
        const { data: players, error: searchError } = await supabase
          .from('players')
          .select('id')
          .or(`name.ilike.%${playerRating.name}%`)
          .limit(1)
